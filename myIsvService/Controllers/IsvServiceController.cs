...
namespace myIsvService.Controllers
{
    ...
    public class IsvServiceController : ControllerBase
    {
        ...
        {
            _logger.LogInformation("Initiating connection creation");

            string tenantId = ISVServiceUtilities.GetTenantId(gcsNotification.Value.First());
            _logger.LogInformation($"TenantId : {tenantId}");

            string aadAppId = ISVServiceUtilities.GetAadAppId(gcsNotification.ValidationTokens.First());
            _logger.LogInformation($"aadAppId : {aadAppId}");

            string token = await GetToken("https://graph.microsoft.com", tenantId, aadAppId);
            _logger.LogInformation($"Token : {token}");

            string tokenForCheckingConnection = await GetToken("https://substrate.office.com/GraphConnectors", tenantId, aadAppId);
            _logger.LogInformation($"Token for checking connection: {tokenForCheckingConnection}");
            ...
            Task.Run(() => ScheduleItemIngestion(externalConnectionBody, token, teamsAppId, tokenForCheckingConnection));
            return "";
        }
        ...
        private async Task ScheduleItemIngestion(ExternalConnectionBody externalConnectionBody, string token, string connectorId, string tokenForCheckingConnection)
        {
            await CheckConnectionStatus(externalConnectionBody, tokenForCheckingConnection, connectorId);
        ...
        public async Task<bool> GetConnection(ExternalConnectionBody externalConnectionBody, string token, string connectorId)
        {
            ...
            try
            {
                var response = await Get($"https://substrate.office.com/GraphConnectors/v1.0/external/connections?$filter=tolower(connectorId) eq '{connectorId}'", token);
                GetConnectionsResponse externalConnections = JsonConvert.DeserializeObject<GetConnectionsResponse>(response);
                bool isConnectionReady = ISVServiceUtilities.IsConnectionReady(externalConnections, externalConnectionBody.id);
        ...
        private async Task<string> GetToken(string resource, string tenantId, string aadAppId)
        {
            _logger.LogInformation($"Getting access token for resource : {resource} tenantId : {tenantId} and clientId : {aadAppId}");
